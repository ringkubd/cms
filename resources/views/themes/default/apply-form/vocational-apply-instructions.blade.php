...
@inject('bangla', 'App\Http\Numbertobangla')
      ...
      <div class="row">
        <div class="col-md-9 col-sm-8 py-3">
          <div class="post-content">
            @php $instruction = $home->get_page_by_id(92,'vocational-training-programme'); @endphp
            <p class="text-center">
            ...
            @if($instruction)
              @php
                $round = $bangla->number($vtpIntake->round);
                $banDate = $bangla->monthtobangla(date('d-F-Y', strtotime($vtpIntake->end_date)));
                $content = str_replace('[round]', $round , $instruction->post_content );
                $content = str_replace('[end_date]', $banDate , $content );
              @endphp
              {!! $content !!}
            @else
              Instructions fail
