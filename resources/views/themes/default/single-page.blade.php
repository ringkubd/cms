        ...
        <div class="col-lg-3 col-md-5 col-sm-6 py-3">
          <div class="sidebar-widget mb-4">
            ...
            <ul class="widget-list">
              @php
                $related_posts = $home->get_related_page($post->post_format, 15);
              @endphp
              @foreach($related_posts as $r_posts)
                @php
                  ...
                  $currentPage = request()->is("{$post->post_format}/{$post->post_slug}");
                  $currentItem = request()->is("{$r_posts->post_format}/{$r_posts->post_slug}");
                @endphp
                <li class="item">
                  <a class="item-link @if($currentPage == $currentItem) active @endif"
                     href="{{$link}}">{{$r_posts->post_title}}</a>
                </li>
              @endforeach
            </ul>
          </div> <!-- .sidebar-widget -->
      ...
      <div class="col-lg-9 col-md-7 col-sm-6 py-3">
        <div id="printContent">
          ...
          <article class="post-content mb-5 clearfix">
            @if($post->attachment)
              @if($post->attachment->attachment_type == 'video')
            ...
            @else
              <div class="single-content-head">
                ...
                <a href="{{url($module->slug ?? '/')}}" class="post-date d-print-none">{{$module->name ?? null}}</a>
              </div>
            ...
            @endif
            @if($post->attachment)
              @if($post->attachment->attachment_type == 'file')
                <a href="{{url('download?form='.$post->attachment->attachment_path)}}" class="btn btn-main">Download</a>
              @endif
            @endif
            @php
