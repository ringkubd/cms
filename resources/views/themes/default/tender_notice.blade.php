...
@section('title', 'Latest Notice of IDB Bhaban 2')
@section('m_title', 'Latest Notice of IDB Bhaban 2')
@section('m_description', 'Latest Notice of IDB Bhaban 2')
...
<div class="container">
  <div class="row">
    ...
    <div class="col-lg-3 col-md-4 col-sm-5 py-3">
      <div class="sidebar-widget mb-4">
        ...
        <ul class="widget-list">
          <li class="item">
            <a class="item-link" href="{{url("tender_notice")}}">All Notice</a>
            <span class="badge badge-secondary float-right m-1">{{$home->count_posts("tender")}}</span>
          </li>
          @foreach($programmes as $programme)
          <li class="item">
            <a class="item-link" href="{{url("notice?type=".$programme->slug)}}">{{$programme->name}}</a>
            <span class="badge badge-secondary float-right m-1">{{$home->count_posts('tender',$programme->slug)}}</span>
          </li>
          @endforeach
        </ul>
      </div> <!-- .sidebar-widget -->
    ...
    <div class="col-lg-9 col-md-8 col-sm-7 py-3">
      <h1 class="area-title">
        <span class="title-text">Notice of IDB Bhaban 2</span>
      ...
      <a href="{{url("tender_notice")}}" class="btn-link">Clear</a>
      @endif
      @php
      $collapse = request()->has("start") ? "show" : "";
      @endphp
      <div class="collapse {{$collapse}}" id="filter">
        <form action="{{url("tender_notice")}}" method="get">
          <div class="row">
